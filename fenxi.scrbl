...
@section{编程语言概览}
编程语言（英语：programming language），是用来定义计算机程序的
...
@subsection{类型系统}
一个程序中专门处理数据的那个系统被称为程序语言的类型系统（type system）；
对型态系统的研究和设计被称为型态理论（type theory）。语言
可以被分为静态型态系统（statically typed systems），例如C++和Java，和
动态型态系统（dynamically typed systems），例如Lisp，JavaScript，Tcl
和 Prolog。
...
阿隆佐·邱奇（Alonzo Church）在1930年代开发的λ演算，是建造自函
数应用的一种计算形式系统。在1937年，艾伦·图灵证明了λ演算和图灵机是
等价的计算模型，展示了λ演算是图灵完备性的。λ演算@cite{Matthias2006}形成
了所有函数式编程语言的基础。

@section{Lisp、Scheme和Racket}
@subsection{历史}
在1958年，约翰·麦卡锡在麻省理工学院发明了Lisp编程语言。

Lisp（历史上拼写为LISP），是具有悠久历史的计算机编程语言家族，它
起源于1958年，是现今第二悠久而仍广泛使用的高级编程语言。Lisp编程语
族已经演变出许多种方言，现代最著名的通用编程方言是Scheme、Common
Lisp 和新近的Clojure。

在1970年代，Guy L. Steele和Gerald Jay Sussman开发了Scheme，如
有影响力的“λ论文集”和经典的1985年教科书《计算机程序的构造和解释》@cite{abelson96}
中所描述的那样。Scheme是使用词法作用域和尾调用优化的第一个Lisp方
言，将函数式编程的影响力提升到更广泛的范围，让更多的编程语言社区接触
到它们。

Matthias Felleisen在1990年代中期创建了PLT研究团队。在1995年1月，
这个团队决定开发一个创建在Scheme函数式编程语言上的教学用编程语
言环境，Mattew Flatt简单的从libscheme中将MrEd(Racket的原始虚拟
机)、wxWidgets与一些其他的免费系统结合，接下来的几年内，由Flatt、
Robby Findler、Shriram Krishnamurthi、Cormac Flanagan和其
他许多人组成的团队推出了DrScheme，在2010年6月7日，PLT Scheme被
重命名为Racket。

@subsection{数据结构}
上文提到，Lisp程序的同像性是通过S-表达式和数据结构的形式相同达到
的，如果我们观察Lisp的数据结构，能发现它们存在一种高度相关性：
@tabular[#:style 'boxed
...
@subsection{优缺点}
不同于现今流行的很多编程语言，S-表达式作为Lisp语法是极其规整的，
非常利于计算机操纵，Lisp通过数据结构和语法的形式相同，故此让Lisp程
序具备一种同像性，Lisp因此得以可以让编程语言可以一些简单的核心函数为
基准，构造起几乎所有的目前流行编程语言所具有的高阶概念，Scheme作为
Lisp 的一种方言，即是遵循极简主义哲学，以一个小型语言核心作为标准，加
上各种强力语言工具（语法糖）来扩展语言本身的典范。

但是Lisp高度统一的语法和数据是一柄双刃剑，S-表达式极为简单高效且
...
我们下面来举例实际说明一下。首先我们看一下相同作用的代码
用Racket和Ruby分别是如何表达的：
@tabular[#:style 'boxed
         #:column-properties '(left left)
         #:row-properties '(bottom-border ())
...
由上表我们可以看出，不同功能的代码在Ruby的实现中是形式各异
的，而其在Racket的代码实现中都以S-表达式的方式表达的。
...
@itemlist[
@item{
make-list用于生成一个列表，列表的内容是N（第一个参数）个V（第
二个参数）；
}
@item{
build-list也是用于生成一个列表，列表的内容则是从0起的N-1个自然
数（N 是第一个参数）。
}
]

然而，从名字上看，make-list和build-list显然都是要构造列表的，但是
...
现代的二进制记数系统由戈特弗里德·莱布尼茨于1679年设计，在
他1703年发表的文章《论只使用符号0和1的二进制算术，兼论其用途及它赋予伏羲
...
1957年诺贝尔物理学奖得主杨振宁曾经在1993年发表过一篇《近代科学进入中国的回顾与前瞻》(LOOKING BACK ON AND FORWARD TO THE
ENTRANCE OF MODERN SCIENCE INTO CHINA）的演讲@cite{yangzhenning93}，进而在2004
年人民大会堂“2004文化高峰论坛”上有过关于“《易经》对中华文化的影
